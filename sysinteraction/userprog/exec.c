...
static bool segment_load(int32_t fd,uint32_t offset,uint32_t filesz,uint32_t vaddr){
    uint32_t vaddr_first_page=0xfffff000&vaddr;
    uint32_t size_left_in_first_page=PG_SIZE-(vaddr&0x00000fff);
    uint32_t occupy_pages=1;
    if(filesz>size_left_in_first_page){
        uint32_t left_size=filesz-size_left_in_first_page;
...
static int32_t load(const char* pathname){
    ...
    uint32_t prog_idx=elf_header.e_phnum;
    while(prog_idx--){
