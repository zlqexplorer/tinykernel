...
static void release_prog_resource(struct task_struct* release_thread){
    ...
    uint32_t user_pde_nr=768,pde_idx=0;
    uint32_t pde=0;
    uint32_t* v_pde_ptr=NULL;

    uint32_t user_pte_nr=1024,pte_idx=0;
    uint32_t pte=0;
    ...
    while(pde_idx<user_pde_nr){
        ...
        pde=*v_pde_ptr;
        if(pde&0x00000001){
            first_pte_vaddr_in_pde=pte_ptr(pde_idx<<22);
            pte_idx=0;
    ...
    while(1){
        ...
        if(child_elem!=NULL){
            ...
            *status=child_thread->exit_status;
            uint32_t child_pid=child_thread->pid;
...
void sys_exit(int32_t status){
    ...
    if(child_thread->parent_pid==-1){
        PANIC("sys_exit: parent pid is -1\n");
    }
