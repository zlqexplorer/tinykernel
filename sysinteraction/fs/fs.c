...
static void partitioin_format(struct partition* part){
...
sb.inode_bitmap_sects?sb.block_bitmap_sects: \
sb.inode_bitmap_sects;
    buf_size=(buf_size>=inode_table_sects?  \
buf_size:inode_table_sects)*SECTOR_SIZE;
    ...
    while(block_bitmap_last_bit--){
        buf[block_bitmap_last_byte]&=~(1<<block_bitmap_last_bit);
    }
    ...
    while(name[0]){
        ...
        if(search_dir_entry(cur_part,parent_dir,name,&dir_e)){
            ...
            if(FT_DIRECTORY==dir_e.f_type){
                ...
                ASSERT(parent_dir->inode->i_no==dir_e.i_no);
...
int32_t sys_open(const char* pathname,uint8_t flags){
    ...
    uint32_t pathname_cnt=path_depth_cnt((char*)pathname);
    ...
    if(pathname_cnt!=path_searched_depth){
        printk("can't access %s: Not a directory, subpath %s is't exist\n", \
pathname,searched_record.searched_path);
        dir_close(searched_record.parent_dir);
        return -1;
    }
    if(inode_no==-1&&!(flags&O_CREAT)){
        printk("in path %s, file %s isn't exist\n", \
searched_record.searched_path,(strchr(searched_record.searched_path,'/')+1));
        dir_close(searched_record.parent_dir);
        return -1;
    }else if(inode_no!=-1&&(flags&O_CREAT)){
...
(strrchr(searched_record.searched_path,'/')+1),flags);
            dir_close(searched_record.parent_dir);
            break;
        default:
        ...
        while(dev_no<2){
            ...
            while(part_idx<12){
                ...
                if(part->sec_cnt!=0){
                    ...
                    if(sb_buf->magic==0x19970508){
                        //printk("%s has filesystem\n",part->name);
                    }else{
                        printk("formatting %s's partition %s......\n",hd->name,part->name);
                        partitioin_format(part);
                    }
                }
                ++part_idx;
                ++part;
...
int32_t sys_write(int32_t fd,const void* buf,uint32_t count){
    ...
    uint32_t gfd=fd_local2global(fd);
    struct file* wr_file=&file_table[gfd];
...
int32_t sys_read(int32_t fd,void* buf,uint32_t count){
    ...
    if(fd<0||fd==stdout_no||fd==stderr_no){
        ...
        uint32_t gfd=fd_local2global(fd);
        ret=file_read(&file_table[gfd],buf,count);
    }
    return ret;
}
...
int32_t sys_lseek(int32_t fd,int32_t offset,uint8_t whence){
    ...
    switch(whence){
        ...
        case SEEK_END:
            ...
            break;
        default:
            printk("sys_lseek: unknown whence\n");
    }
...
int32_t sys_unlink(const char* pathname){
    ...
    uint32_t file_idx=0;
    while(file_idx<MAX_FILE_OPEN&&(file_table[file_idx].fd_inode==NULL||(uint32_t)inode_no!=file_table[file_idx].fd_inode->i_no)){
        ++file_idx;
    }
...
struct dir* sys_opendir(const char* name){
    ...
    if(inode_no==-1){
       printk("In %s,sub path %s not exist\n",name,searched_record.searched_path);
        ...
        ASSERT(searched_record.file_type!=FT_UNKNOWN);
...
char* sys_getcwd(char* buf,uint32_t size){
    ...
    //char io_buf[SECTOR_SIZE]={0};
    if(io_buf==NULL){
        return NULL;
    }
    struct task_struct* cur=running_thread();
    int32_t parent_inode_nr=0,child_inode_nr=cur->cwd_inode_nr;
...
int32_t sys_chdir(const char* path){
    ...
    if(inode_no!=-1){
        if(searched_record.file_type==FT_DIRECTORY){
            ...
            printk("sys_chdir: %s is regular or other\n",path);
        }
    }
    return ret;
}
