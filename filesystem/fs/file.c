...
int32_t get_free_slot_int_global(void){
    uint32_t fd_idx=3;
...
int32_t file_create(struct dir* parent_dir,char* filename,uint8_t flags){
    ...
    if(inode_no==-1){
        printk("in file_creat: alloc inode failed\n");
        return -1;
    }

    struct inode* new_file_inode=(struct inode*)sys_malloc(sizeof(struct inode));
    if(new_file_inode==NULL){
        printk("in file_creat: sys_malloc for inode failed\n");
        rollback_step=1;
        goto rollback;
    }
    inode_init(inode_no,new_file_inode);

    int fd_idx=get_free_slot_int_global();
    if(fd_idx==-1){
        printk("exceed max open files\n");
        rollback_step=2;
        goto rollback;
...
int32_t file_open(uint32_t inode_no,uint8_t flags){
    int32_t fd_idx=get_free_slot_int_global();
    if(fd_idx==-1){
...
int32_t file_write(struct file* file,const void* buf,uint32_t count){
    if(file->fd_inode->i_size+count>BLOCK_SIZE*140){
        printk("exceed max file size 71680 bytes\n");
        return -1;
    }
    ...
    if(add_blocks<=0){
        if(file_will_use_blocks<=12){
            ...
            ide_read(cur_part->my_disk,indirect_block_table,(void*)(all_blocks+12),1);
        }
    }else{
        if(file_will_use_blocks<=12){
            block_idx=file_has_used_blocks-1;
            ...
            ide_write(cur_part->my_disk,indirect_block_table,(void*)(all_blocks+12),1);
...
int32_t file_read(struct file* file,void* buf,uint32_t count){
    ...
    uint32_t block_read_end_idx=(file->fd_pos+count)/BLOCK_SIZE;
